...
# from pet.api import serializers_old
...
#             raise Exception 
#         petowner = PetOwner.objects.filter(email=self.request.user.email).first()
#         if petowner is None:
#             raise PetOwner.DoesNotExist
#         return petowner
...
#     serializer_class = serializers_old.PetSaveSerializer
...
#                 return httputils.response_as_json(serializers_old.PetSerializer(pet).data, status.HTTP_201_CREATED)
...
#             return httputils.response_as_json(serializers_old.PetSerializer(pet).data)
#         except PetOwner.DoesNotExist:
#             return httputils.response_bad_request_400("Pet Owner does not exists.")
...
#             try:
#                 pet = serializer.save()
#                 return httputils.response_as_json(serializers_old.PetSerializer(pet).data, status.HTTP_202_ACCEPTED)
#             except Exception as e:
#                 return httputils.response_bad_request_400(str(e))
#         return httputils.response_as_json(serializer.errors, status.HTTP_400_BAD_REQUEST)
...
#     serializer_class = serializers_old.HistoricoMedicoSerializer
#     permission_classes = [IsAuthenticated]
#     authentication_classes = [SessionAuthentication]
...
#             return httputils.response_as_json(serializers_old.HistoricoMedicoSerializer(medical_history).data)
#         except Exception as e:
#             return httputils.response_bad_request_400(str(e))
    
# class PetMedicalHistoryVacinaViewSet(PetBaseViewSet, ResponseMixin):
#     serializer_class = serializers_old.VacinaAdministradasSaveSerializer
#     permission_classes = [IsAuthenticated]
#     authentication_classes = [SessionAuthentication]
...
#         action_serializers = {
#             "update_vacina": serializers_old.VacinaAdministradasUpdateSerializer
#         }
...
#             return httputils.response_as_json(serializers_old.VacinasAdministradasSerializer(vacina_administrada_updated).data, status.HTTP_202_ACCEPTED)

# class PetMedicalHistoriyVermifugoViewSet(PetBaseViewSet, ResponseMixin):
#     serializer_class = serializers_old.VermifugosAdministradosSaveSerializer
#     permission_classes = [IsAuthenticated]
#     authentication_classes = [SessionAuthentication]
...
#     def get_serializer_class(self):
#         action_serializers = {
#             "update_vermifugo": serializers_old.VermifugosAdministradosUpdateSerializer
#         }
#         return action_serializers.get(self.action, super().get_serializer_class())
...
#             return httputils.response_as_json(serializers_old.VermifugosAdministradosSerializer(vermifugo_updated).data, status.HTTP_202_ACCEPTED)
